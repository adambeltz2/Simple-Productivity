...
*   **Data & Persistence:**
    *   No database, no backend API. All project/note data lives in the browser's `localStorage`; that is the source of truth.
    *   Each project is a record with `title`, `active`, `sortOrder`, and a markdown `body`. Tasks and follow-ups are not separate records — they are derived by parsing the body: `- [ ] ...` checkbox syntax for tasks, an inline `!followup: ...` marker for follow-ups. The dashboard is a read-only projection over the notes, never a second source of truth.
